...
/*QImage brightness_contrast(Mat input, double alpha, int beta)
{
...
QImage brightness_contrast(QImage &input, double alpha, int beta){
    ...
    for(int y = 0; y < height; ++y){
        for(int x = 0; x < width; ++x){
            QColor pix = temp.pixel(x,y);
            /*int r = overflow(pix.red()*alpha);
            int g = overflow(pix.green()*alpha);
            int b = overflow(pix.blue()*alpha);*/
            double r = overflow(((pix.red()/255 - 0.5)*alpha + 0.5)*255);
            double g = overflow(((pix.green()/255 - 0.5)*alpha + 0.5)*255);
            double b = overflow(((pix.blue()/255 - 0.5)*alpha + 0.5)*255);
            int a = pix.alpha();
            QRgb rgb = qRgba(r, g, b, a);
            temp.setPixel(x,y,rgb);
        }
